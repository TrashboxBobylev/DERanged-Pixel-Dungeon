...
public class HighGrass {
	...
	public static void trample( Level level, int pos ) {
		...
		if (level.map[pos] == Terrain.FURROWED_GRASS){
			if (ch instanceof Hero && ((Hero) ch).canFurrowGrass()){
				//Do nothing
				freezeTrample = true;
			} else {
			...
			if (ch != null) {
				...
				if (ch instanceof Hero && ((Hero) ch).hasTalent(Talent.NATURES_BOUNTY, Talent.ROYAL_PRIVILEGE)){
					int berriesAvailable = 2 + 2*((Hero) ch).pointsInTalent(Talent.NATURES_BOUNTY, Talent.ROYAL_PRIVILEGE);
					...
					if (berriesAvailable > 0) {
						int targetFloor = 2 + 2 * ((Hero) ch).pointsInTalent(Talent.NATURES_BOUNTY, Talent.ROYAL_PRIVILEGE);
			...
			if (ch instanceof Hero && Dungeon.hero.hasTalent(Talent.NATURE_FRIENDLY, Talent.THE_PROTECTOR)) {
				Barkskin.conditionallyAppend(hero, 1+2*Dungeon.hero.pointsInTalent(Talent.NATURE_FRIENDLY, Talent.THE_PROTECTOR), 3);
			}
		}
