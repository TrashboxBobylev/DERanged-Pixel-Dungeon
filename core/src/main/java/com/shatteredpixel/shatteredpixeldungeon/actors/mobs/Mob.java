...
public abstract class Mob extends Char {
	...
	public float speed() {
		...
		speed /= ((Dungeon.isChallenged(Challenges.RANDOM_HP) && scaleFactor != 1) ? (0.8f * scaleFactor) : 1);
		return speed;
	}
	...
	public void die( Object cause ) {
		...
		if ((this.alignment != Alignment.ALLY) && (cause == hero || cause instanceof Wand || cause instanceof DirectableAlly)) {
			if (Dungeon.hero.hasTalent(Talent.LETHAL_RAGE, Talent.RK_BERSERKER)){
			...
			if (hero.hasTalent(Talent.RK_GUNSLINGER) || hero.canHaveTalent(Talent.SOUL_BULLET)) {
				Buff.affect(hero, InfiniteBullet.class, hero.shiftedPoints(Talent.SOUL_BULLET, Talent.RK_GUNSLINGER));
			}

			if (hero.hasTalent(Talent.ADRENALINE_SURGE)) {
				Buff.prolong(hero, Adrenaline.class, 1+3*hero.pointsInTalent(Talent.ADRENALINE_SURGE));
			}
			...
			if (hero.hasTalent(Talent.KINETIC_BATTLE, Talent.EXTRA_POWER)) {
