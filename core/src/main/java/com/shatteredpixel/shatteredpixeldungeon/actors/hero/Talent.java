...
public enum Talent {
	...
	public static class RejuvenatingStepsCooldown extends FlavourBuff{
		...
		public float iconFadePercent() { return GameMath.gate(0, visualcooldown() / (15 - 5* hero.pointsInTalent(REJUVENATING_STEPS, POWER_WITHIN)), 1); }
	};
	...
	public static class ArmorEmpower extends Buff {
		...
		@Override
		public float iconFadePercent() {
			float max = 1 + hero.pointsInTalent(Talent.TOUGH_MEAL, ROYAL_FOCUS);
	...
	public static void onTalentUpgraded( Hero hero, Talent talent ){
		...
		//archer
		if ((talent == MAKESHIFT_BOW || talent == RK_BOWMASTER) && hero.heroClass.is(HeroClass.ARCHER)) {
			BowWeapon bow = null;
			switch (hero.pointsInTalent(Talent.MAKESHIFT_BOW, RK_BOWMASTER)) {
				case 0: default:
					break;
				case 1:
					bow = new Bow();
	...
	public static void onFoodEaten( Hero hero, float foodVal, Item foodSource ){
		...
		if (hero.hasTalent(Talent.FORCE_SAVING, ROYAL_FOCUS)) {
	...
	public static int onAttackProc( Hero hero, Char enemy, int damage ){
		...
		if (hero.hasTalent(Talent.WATER_FRIENDLY, EXTRA_POWER) && level.map[hero.pos] == Terrain.WATER) {
		...
		if (hero.hasTalent(Talent.STRONG_NEXUS, RK_SAVIOR)) {
			for (Mob mob : level.mobs.toArray( new Mob[0] )) {
				if (mob.alignment == Char.Alignment.ALLY && level.heroFOV[mob.pos]) { // 아군이 영웅의 시야 내에 있을 때
					int healAmt = 3 * hero.pointsInTalent(Talent.STRONG_NEXUS, RK_SAVIOR) - level.distance(hero.pos, mob.pos) + 1;
					if (healAmt > 0) {
						mob.heal(healAmt);
					}
				}
	...
	public static void initClassTalents( HeroClass cls, ArrayList<LinkedHashMap<Talent, Integer>> talents, LinkedHashMap<Talent, Talent> replacements ){
		...
		switch (cls){
			...
			case CLERIC:
				...
				break;
			case GUNNER:
				Collections.addAll(tierTalents, RELOADING_MEAL, GUNNERS_INTUITION, SPEEDY_MOVE, SAFE_RELOAD, CLOSE_COMBAT, CRAFTMANSHIP);
				break;
			case SAMURAI:
				Collections.addAll(tierTalents, BASIC_PRACTICE, MASTERS_INTUITION, DRAWING_ENHANCE, PARRING, ADRENALINE_SURGE, PRECISE_RESTORATION);
				break;
			case ADVENTURER:
	...
	public static void initSubclassTalents( HeroSubClass cls, ArrayList<LinkedHashMap<Talent, Integer>> talents ){
		...
		switch (cls){
			...
			case SHARPSHOOTER:
				...
				break;
			case KING:
				Collections.addAll(tierTalents, RK_BERSERKER, RK_GLADIATOR, RK_VETERAN, RK_BATTLEMAGE, RK_WARLOCK, RK_WIZARD,
						RK_ASSASSIN, RK_FREERUNNER, RK_CHASER, RK_SNIPER, RK_WARDEN, RK_FIGHTER,
