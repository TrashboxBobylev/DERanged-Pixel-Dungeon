...
public class CloakOfShadows extends Artifact {
	...
	public void charge(Hero target, float amount) {
		...
		if (charge < chargeCap) {
			if (!isEquipped(target)) amount *= 0.75f*target.pointsInTalent(Talent.LIGHT_CLOAK, Talent.RK_FREERUNNER)/3f;
			...
			if (charge < chargeCap && !cursed && target.buff(MagicImmune.class) == null) {
				if (activeBuff == null && Regeneration.regenOn()) {
					...
					if (!isEquipped(Dungeon.hero)){
						chargeToGain *= 0.75f*Dungeon.hero.pointsInTalent(Talent.LIGHT_CLOAK, Talent.RK_FREERUNNER)/3f;
					}
	...
	public class cloakStealth extends ArtifactBuff{
		...
		public float iconFadePercent() {
			return (maxTurns + ((Hero) target).pointsInTalent(Talent.CAUTIOUS_PREP, Talent.RK_ASSASSIN) - turnsToCost) / (maxTurns + ((Hero) target).pointsInTalent(Talent.CAUTIOUS_PREP));
		}
		...
		public boolean act(){
			...
			if (turnsToCost <= 0){
				...
				if (charge < 0) {
					...
					turnsToCost = 4 + ((Hero) target).pointsInTalent(Talent.CAUTIOUS_PREP, Talent.RK_ASSASSIN);
				}
				updateQuickslot();
			}
