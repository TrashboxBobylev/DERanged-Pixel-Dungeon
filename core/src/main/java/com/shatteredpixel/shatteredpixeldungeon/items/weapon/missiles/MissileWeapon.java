...
abstract public class MissileWeapon extends Weapon {
	...
	public int throwPos(Hero user, int dst) {
		...
		if ((!(this instanceof SpiritBow.SpiritArrow) && Random.Int(3) < user.pointsInTalent(Talent.SHARED_ENCHANTMENT, Talent.RK_SNIPER))){
		...
		if ((this instanceof Gun.Bullet && (Dungeon.level.passable[dst] || Dungeon.level.avoid[dst])
                && Dungeon.level.distance(user.pos, dst) <= 1+Dungeon.hero.pointsInTalent(Talent.STREET_BATTLE, Talent.RK_OUTLAW) + (hero.hasTalent(Talent.STREET_BATTLE) ? 1 : 0))
                || (projecting > 0
	...
	public float durabilityPerUse( int level ){
		...
		if (Dungeon.hero != null && Dungeon.hero.hasTalent(Talent.DURABLE_PROJECTILES, Talent.PURSUIT)){
			usages *= 1f + (1+Dungeon.hero.pointsInTalent(Talent.DURABLE_PROJECTILES, Talent.PURSUIT))/6f;
		}
		if (holster) {
