...
public class RingOfEnergy extends Ring {
	...
	public static float wandChargeMultiplier( Char target ){
		...
		if (target instanceof Hero && !((Hero) target).heroClass.is(HeroClass.CLERIC) && ((Hero) target).hasTalent(Talent.LIGHT_READING, Talent.RK_PALADIN)){
			bonus *= 1f + (0.2f * ((Hero) target).pointsInTalent(Talent.LIGHT_READING, Talent.RK_PALADIN)/3f);
		}
	...
	public static float artifactChargeMultiplier( Char target ){
		...
		if (target instanceof Hero && !((Hero) target).heroClass.is(HeroClass.ROGUE) && ((Hero) target).hasTalent(Talent.LIGHT_CLOAK, Talent.RK_FREERUNNER)){
			bonus *= 1f + (0.2f * ((Hero) target).pointsInTalent(Talent.LIGHT_CLOAK, Talent.RK_FREERUNNER)/3f);
		}
