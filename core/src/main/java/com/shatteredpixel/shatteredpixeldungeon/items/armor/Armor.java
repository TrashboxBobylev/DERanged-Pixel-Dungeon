...
public class Armor extends EquipableItem {
	...
	public ArrayList<String> actions(Hero hero) {
		...
		if (!isEquipped(hero) && hero.heroClass.is(HeroClass.GUNNER)) {
	...
	public boolean doEquip( Hero hero ) {
		...
		// 15/25% chance
		if (!hero.heroClass.is(HeroClass.CLERIC) && hero.hasTalent(Talent.HOLY_INTUITION)
				&& cursed && !cursedKnown
		...
		if (hero.belongings.armor == null || hero.belongings.armor.doUnequip( hero, true, false )) {
			...
			if (Dungeon.hero.heroClass.is(HeroClass.WARRIOR) && checkSeal() == null){
	...
	public int DRMax(int lvl){
		...
		if (hero != null) {
			if (hero.hasTalent(Talent.ARMOR_ADAPTION, Talent.KINGS_WRATH) && this.STRReq() < hero.STR()) {
				max += Math.round((hero.STR() - this.STRReq())*(0.5f+0.5f*hero.pointsInTalent(Talent.ARMOR_ADAPTION, Talent.KINGS_WRATH)));
			}
		}
		if (lvl > max) {
			return ((lvl - max) + 1) / 2;
	...
	public static abstract class Glyph implements Bundlable {
		...
		public static float genericProcChanceMultiplier( Char defender ){
			...
			if (Dungeon.hero.alignment == defender.alignment
					&& Dungeon.hero.buff(AuraOfProtection.AuraBuff.class) != null
					&& (Dungeon.level.distance(defender.pos, Dungeon.hero.pos) <= 2 || defender.buff(LifeLinkSpell.LifeLinkSpellBuff.class) != null)){
				multi += 0.25f + 0.25f*Dungeon.hero.pointsInTalent(Talent.AURA_OF_PROTECTION, Talent.RK_PALADIN);
			}
