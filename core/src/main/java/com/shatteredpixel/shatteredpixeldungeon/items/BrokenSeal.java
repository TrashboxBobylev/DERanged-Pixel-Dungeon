...
public class BrokenSeal extends Item {
	...
	public int maxShield( int armTier, int armLvl ){
		...
		return 3 + 2*armTier + Dungeon.hero.pointsInTalent(Talent.IRON_WILL, Talent.NOBLE_CAUSE);
	}
	...
	public static class WarriorShield extends ShieldBuff {
		...
		public synchronized int maxShield() {
			...
			if (!((Hero)target).heroClass.is(HeroClass.WARRIOR) && ((Hero) target).hasTalent(Talent.IRON_WILL, Talent.NOBLE_CAUSE)){
				return ((Hero) target).pointsInTalent(Talent.IRON_WILL, Talent.NOBLE_CAUSE);
			}
